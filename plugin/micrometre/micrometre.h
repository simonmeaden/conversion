 ...
 * \class Micrometre
 *
 * \brief defines a length in micrometres.
 *
 * Micrometres (or 10 \sup{-6} metres) defines a length in  the \a Conversion utility.
 *
 * \author Simon Meaden
 * \date   20 July 2016
 */

class CONVERSIONSHARED_EXPORT Micrometre : public QObject, public AbstractLengthConversion {
        Q_OBJECT
        Q_PLUGIN_METADATA( IID IConversionInterface_iid )
        Q_INTERFACES( Conversion::IConversionInterface )
    public:
        Micrometre( ) :
            AbstractLengthConversion( QStringLiteral("1E18"),
                                      QStringLiteral("Micrometre"),
                                      QStringLiteral("μm"),
                                      tr("Micrometre = 10<sup>-6</sup> metres")) {
        }
