 ...
 * \class Millimetre
 *
 * \brief defines a length in millimetres.
 *
 * Millimetres (or 10 \sup{-3} metres) defines a length in  the \a Conversion utility.
 *
 * \author Simon Meaden
 * \date   20 July 2016
 */

class CONVERSIONSHARED_EXPORT Millimetre : public QObject, public AbstractLengthConversion {
        Q_OBJECT
        Q_PLUGIN_METADATA( IID IConversionInterface_iid )
        Q_INTERFACES( Conversion::IConversionInterface )
    public:
        Millimetre( ) :
            AbstractLengthConversion( QStringLiteral("1E21"),
                                      QStringLiteral("Millimetre"),
                                      QStringLiteral("mm"),
                                      tr("Millmetres = 1/1000 metre")) {
        }
