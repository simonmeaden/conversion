 ...
 * \class Decimetre
 *
 * \brief defines a length in decimetres.
 *
 * Decimetres (or 10 \sup{-6} metres) defines a length in  the \a Conversion utility.
 *
 * \author Simon Meaden
 * \date   20 July 2016
 */

class CONVERSIONSHARED_EXPORT Decimetre : public QObject, public AbstractLengthConversion {
        Q_OBJECT
        Q_PLUGIN_METADATA( IID IConversionInterface_iid )
        Q_INTERFACES( Conversion::IConversionInterface )
    public:
        Decimetre( ) :
            AbstractLengthConversion( QStringLiteral("1E23"),
                                      QStringLiteral("Decimetre"),
                                      QStringLiteral("dm"),
                                      tr("1 decimetre = 1/10 metre")) {
        }
