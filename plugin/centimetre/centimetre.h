 ...
 * \class Centimetre
 *
 * \brief defines a length in centimetres.
 *
 * Centimetres (or 10 \sup{-6} metres) defines a length in  the \a Conversion utility.
 *
 * \author Simon Meaden
 * \date   20 July 2016
 */

class CONVERSIONSHARED_EXPORT Centimetre : public QObject, public AbstractLengthConversion {
        Q_OBJECT
        Q_PLUGIN_METADATA( IID IConversionInterface_iid )
        Q_INTERFACES( Conversion::IConversionInterface )
    public:
        Centimetre( ) :
            AbstractLengthConversion( QStringLiteral("1E22"),
                                      QStringLiteral("Centimetre"),
                                      QStringLiteral("cm"),
                                      tr("1 Centimetre, 1/100 metre")) {
        }
