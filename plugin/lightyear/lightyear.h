 ...
 * \class LightYear
 *
 * \brief defines a length in light years.
 *
 * LightYears (or 9,460,730,472,580.88 km ) defines a length in  the \a Conversion utility.
 *
 * \author Simon Meaden
 * \date   20 July 2016
 */

class CONVERSIONSHARED_EXPORT LightYear : public QObject, public AbstractLengthConversion {
        Q_OBJECT
        Q_PLUGIN_METADATA( IID IConversionInterface_iid )
        Q_INTERFACES( Conversion::IConversionInterface )
    public:
        LightYear( ) :
            AbstractLengthConversion( QStringLiteral("94607304725808E26"),
                                      QStringLiteral("LightYear"),
                                      QStringLiteral("ly"),
                                      tr("A light year is a bloody long way.")) {
        }
